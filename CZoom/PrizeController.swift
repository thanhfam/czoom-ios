...
//  ZoomGiaoThong
//
//  Created by Thanh Phạm on 2/1/18.
...
class PrizeController: MyViewController, UITableViewDelegate, UITableViewDataSource {
	...
	override func viewDidLoad() {
		...
		self.navigationItem.title = LocalizedString("Gift")
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! PrizeCell
		cell.labelTitle.text = dataArray[indexPath.row]["title"] as! String
		cell.labelPrice.text = dataArray[indexPath.row]["lead"] as! String
		cell.imageAvatar.setImage(url: URL(string: dataArray[indexPath.row]["avatar_url"] as! String)!)
